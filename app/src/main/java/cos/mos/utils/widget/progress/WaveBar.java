...
public class WaveBar extends View {
     ...
     * 6.MULTIPLY:正片叠底像素颜色值=源图像素颜色值 x 目标图像素颜色值 ÷ 255
     * 7.OVERLAY:叠加
     * 8.SCREEN:滤色，色调均和,保留两图中较白的部分，遮盖较暗的部分
     * 9.XOR:两图相交处之外绘制它们，相交处受对应alpha和色值影响，如果:不透明=>相交处不绘制
     * <p>
     * 1.DST:只显示目标图像
     * 2.DST_OVER:将目标图像 放在源图像上方
     * 3.DST_ATOP:两图相交处绘制【下层图】，不相交处绘制【上层图】，效果受两图alpha影响
     * 5.DST_IN:只在两图相交处绘制【下层图】，效果受源图对应地方alpha影响
     * 6.DST_OUT:只在两图不相交处绘制【下层图】,相交处根据源图alpha进行过滤,源图:不透明=>完全过滤,全透明=>不过滤
     * <p>
     * 1.SRC:只显示源图像

...
//    private Bitmap bmp = BitmapFactory.decodeResource(App.getInstance().getResources(), R.drawable.ic_snow);
    ...
    public static SnowFlake create(int width, int height, Paint paint) {
        ...
        int flakeSize = random.getRandom(FLAKE_SIZE_LOWER, FLAKE_SIZE_UPPER);
        return new SnowFlake(random, position, angle, increment, flakeSize, paint);
    }

    private SnowFlake(Random random, Point position, float angle, float increment, int flakeSize, Paint paint) {
        this.random = random;
...
//        size = new Rect(0, 0, flakeSize, flakeSize);
//        dst = new Rect(0, 0, position.x, position.y);
//        canvas.drawBitmap(bmp, size, dst, paint);
    }
}

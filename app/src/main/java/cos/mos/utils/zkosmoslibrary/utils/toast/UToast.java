...
import cos.mos.utils.zkosmoslibrary.utils.toast.style.ToastBlackStyle;
import cos.mos.utils.zkosmoslibrary.utils.toast.style.ToastQQStyle;
import cos.mos.utils.zkosmoslibrary.utils.toast.style.ToastWhiteStyle;
...
public final class UToast {
    ...
    /**
     * 检查吐司状态，如果未初始化请先调用{@link UToast#init(Application)}
     */
    private static void checkToastState() {
        //吐司工具类还没有被初始化，必须要先调用init方法进行初始化
        if (sToast == null) {
            throw new IllegalStateException("UToast has not been initialized");
        }
    }
